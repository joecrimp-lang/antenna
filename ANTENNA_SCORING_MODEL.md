...
### 14.6 Historical backfill

Both `signal_intent_score` (§14.1) and `theme_scores`/`theme_score_snapshots` (§14.2–§14.4) were introduced after signals already existed in the database — those older rows simply have `null` in the new fields (and, for anything predating Build Chunk 1, in the v0.1 classification fields too). `scripts/backfillIntelligence.ts` is a one-off, local-only script that fills these in — never classifying more speculatively than `lib/research.ts`'s existing methodology, never recomputing anything that's already populated, and never touching the live pipeline. Full detail: `ANTENNA_BACKFILL_PROPOSAL.md` and the "Historical backfill" section of `HANDOVER.md`.

One methodology note specific to backfilling: `signal_intent_score`'s recency component (§14.1) is computed against each signal's own **original** `created_at`, not the date the backfill happens to run — this keeps "how fresh was the evidence when Antenna found it" a permanent, reproducible property of the signal, scored identically whether that happened live or via a later backfill.
